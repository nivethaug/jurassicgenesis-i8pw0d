...
def restart_pm2(project_name: str = None):
    ...
    Args:
        project_name: Project name (uses jurassicgenesis-i8pw0d placeholder by default)
    """
    print("\n" + "="*50)
    print("PM2 RESTART")
    print("="*50)
    ...
    if not project_name:
        project_name = "jurassicgenesis-i8pw0d"
    
    return run(f"sudo pm2 restart jurassicgenesis-i8pw0d-frontend")
...
def main():
    ...
    if not args.no_restart and success:
        restart_pm2()  # Uses jurassicgenesis-i8pw0d placeholder
        reload_nginx()
